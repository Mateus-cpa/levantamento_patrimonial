...
def pagina_principal():
    ...
    st.set_page_config(
        ...
        page_icon='📊',
        layout='wide')
     # se o arquivo data_bronze/lista_bens-processada-<UG>.csv existir, mostrar o conteúdo
    if os.path.exists(f'data_bronze/lista_bens-processado-{st.session_state.selected_ug}.csv'):
        st.title(f'Status da Base Processada da UG {st.session_state.selected_ug}')
        while ('selected_ug' not in st.session_state) and st.session_state.get('selected_ug', 'Selecione uma UG') == 'Selecione uma UG':
            st.session_state.selected_ug = st.selectbox('Selecione a UG', options=['Selecione uma UG',
                                                                                'SRAC','SRAL','SRAP',
                                                                                'SRAM','SRBA','SRCE',
                                                                                'CGAD','SRDF','DITEC',
                                                                                'DIREN','DTI','SRES',
                                                                                ...
                                                                                'SRRR','SRSC','SRSP',
                                                                                'SRSE','SRTO'])
            st.info('UG selecionada. Por favor, recarregue a página para continuar')
        #ler base processada
        ...
        st.warning('Arquivo da UG não encontrado.')
